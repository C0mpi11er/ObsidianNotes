1.find out if vuln to sqli attack by trying stuff like ' or " with tracking cookies or category most times one can try e.g category=3 then try category 2-1 if still evaluates to 3 know there's an sql data base that does it 
or maybe put a space inbetwen the words like category=stock, try  sto''ck if it eval to stock  then know there a database there ...


2.start with the ORDER by 1 pay load to see whats goin on
generally itereat through all type of sql vuln to know the one that works


3 . first find out the data base [[Port_SQLI_Part3 CHEAT]]

4 exploit with knowldege of know data base
