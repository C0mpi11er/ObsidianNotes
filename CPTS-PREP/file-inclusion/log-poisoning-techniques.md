...
> [!ABSTRACT] Overview
> This guide covers advanced log poisoning techniques that combine Local File Inclusion (LFI) with log file contamination to achieve remote code execution by injecting malicious code into log files for later inclusion and execution.

## Prerequisites

- **[!INFO]** LFI vulnerability allowing access to log files.
- **[!NOTE]** Ability to control logged data (User-Agent, HTTP headers, SSH attempts, etc.).
- **[!INFO]** Web server with write permissions to log files.
